...
export default function VehicleServicePage({ label }: { label: string }) {
  ...
  const plans = isAtv ? atvPlans : atvPlans.map((plan) => ({ ...plan, id: plan.id.replace("atv", label.toLowerCase().replace(" history report", "")), name: plan.name.replace("ATV", label.replace(" History Report", "")), features: plan.features.map((feature) => feature.replaceAll("ATV", label.replace(" History Report", ""))) }));
  return <main className="vehicle-service-page">
    ...
    <section id="vehicle-pricing" className="vehicle-pricing-section section-pad"><div className="container"><div className="vehicle-pricing-heading"><span className="eyebrow">REPORT OPTIONS / {isAtv ? "ATV" : "VEHICLE"}</span><h2>Choose your report level.</h2></div><div className="vehicle-plan-grid">{plans.map((plan) => <article className="vehicle-plan-card" key={plan.name}><h3>{plan.name}</h3><div className="vehicle-plan-price"><span>£</span>{plan.price}</div><small>{plan.delivery}</small><ul>{plan.features.map((feature) => <li key={feature}><Check size={11} />{feature}</li>)}</ul><button className="vehicle-buy-button" onClick={() => { saveCart(addCartItem(loadCart(), { id: plan.id, name: plan.name, price: Number(plan.price), href: plan.href })); navigate("/checkout"); }}>Add to cart <ArrowUpRight size={12} /></button></article>)}</div></div></section>
