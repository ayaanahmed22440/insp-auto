...
export default function NotFound() {
  ...
  const adminPath = window.location.pathname === "/admin/login" || window.location.pathname === "/admin" || window.location.pathname.startsWith("/admin/");

  if (adminPath) {
    return (
      <Suspense fallback={<div className="admin-loading" role="status">Checking secure session…</div>}>
        {window.location.pathname === "/admin/login" ? <AdminLogin /> : <AdminApp />}
      </Suspense>
    );
  }
