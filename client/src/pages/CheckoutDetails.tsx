  ...
  return (
    <main className="checkout-details-page">
      <style>{`
        ...
        .checkout-notice svg{color:#7ea800;flex:none}.checkout-notice strong{margin-left:auto;background:#052d8f;color:#fff;padding:8px 13px;font-size:12px;white-space:nowrap;cursor:pointer}
        .checkout-heading{font-size:25px;line-height:1.15;margin:0 0 16px;font-weight:700;color:#17213a}.checkout-form{background:#fff}
        .checkout-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px 26px}.checkout-field{display:flex;flex-direction:column;gap:6px;margin-bottom:11px}.checkout-field.full{grid-column:1/-1}
        .checkout-field label,.checkout-checkbox label{font-size:12px;line-height:1.4;color:#27334d}.checkout-field label::after{content:" *";color:#e3262e}
        ...
        .checkout-field input:focus{box-shadow:0 0 0 2px rgba(23,61,155,.12)}.checkout-checkbox{display:flex;gap:8px;align-items:flex-start;margin:7px 0;color:#27334d}
        .checkout-checkbox input{width:14px;height:14px;margin:1px 0 0;accent-color:#143b97;flex:none}.checkout-checkbox label{cursor:pointer;line-height:1.75}.checkout-required{color:#e3262e}
        .checkout-order-title{font-size:21px;margin:20px 0 12px;color:#17213a}.checkout-order-table{width:100%;border-collapse:collapse;border:2px solid #173d9b;font-size:12px}
        .checkout-order-table th,.checkout-order-table td{border:1px solid #173d9b;padding:8px 9px;text-align:left}.checkout-order-table th:last-child,.checkout-order-table td:last-child{text-align:left;width:27%}
        .checkout-order-table thead th{font-weight:700;background:#fff}.checkout-order-table .total-row td{font-weight:700}.checkout-payment{margin-top:16px;background:#f0eff5;border-radius:3px;overflow:hidden}
        .checkout-payment-head{padding:13px 10px;border-bottom:1px solid #173d9b;font-size:12px;color:#26324a}.checkout-payment-body{padding:10px;display:flex;align-items:center;gap:8px;font-size:12px;color:#46516a}.checkout-payment-body svg{color:#143b97}
      ...
      <div className="checkout-details-wrap">
        <div className="checkout-notice"><CheckCircle2 size={16} aria-hidden="true" /><span><strong style={{display:"inline",background:"transparent",color:"inherit",padding:0,fontWeight:500,cursor:"default"}}>Your report has been selected</strong></span></div>
        <h1 className="checkout-heading">Billing details</h1>
        <form className="checkout-form" onSubmit={submit} noValidate>
          <div className="checkout-grid">
            <div className="checkout-field"><label htmlFor="first-name">First name</label><input id="first-name" value={firstName} onChange={(e)=>setFirstName(e.target.value)} autoComplete="given-name" required /></div>
            <div className="checkout-field"><label htmlFor="last-name">Last name</label><input id="last-name" value={lastName} onChange={(e)=>setLastName(e.target.value)} autoComplete="family-name" required /></div>
            <div className="checkout-field full"><label htmlFor="phone">Phone</label><input id="phone" value={phone} onChange={(e)=>setPhone(e.target.value)} autoComplete="tel" required /></div>
            <div className="checkout-field full"><label htmlFor="email">Email address</label><input id="email" type="email" value={email} onChange={(e)=>setEmail(e.target.value)} autoComplete="email" required /></div>
            <div className="checkout-field full"><label htmlFor="vin">VIN / Registration / HIN number</label><input id="vin" value={vin} onChange={(e)=>setVin(e.target.value.toUpperCase())} autoComplete="off" required /></div>
          </div>
          <div className="checkout-checkbox"><input id="consent-one" type="checkbox" checked={consentOne} onChange={(e)=>setConsentOne(e.target.checked)} required /><label htmlFor="consent-one">I confirm that I am making this purchase voluntarily and that INSP AUTO has not pressured or coerced me into purchasing this report. <span className="checkout-required">*</span></label></div>
          <div className="checkout-checkbox"><input id="consent-two" type="checkbox" checked={consentTwo} onChange={(e)=>setConsentTwo(e.target.checked)} required /><label htmlFor="consent-two">I acknowledge that payment for this order is processed by our authorized third-party payment provider, Whop, and that payment processing is subject to the provider's applicable terms. <span className="checkout-required">*</span></label></div>
