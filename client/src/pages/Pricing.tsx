...
export default function Pricing() {
  ...
  return <main className="pricing-page">
    ...
    <section id="pricing-plans" className="pricing-plans section-pad"><div className="container"><Reveal className="pricing-section-intro"><span className="eyebrow">REPORT OPTIONS / UK BUYERS</span><h2>Clear coverage. <em>Useful context.</em></h2></Reveal><div className="premium-plan-grid">{plans.map((plan) => <Reveal className={`premium-plan-card ${plan.popular ? "is-popular" : ""}`} key={plan.key}>{plan.popular && <span className="popular-badge">MOST POPULAR</span>}<span className="premium-plan-label">{plan.label}</span><h3>{plan.name}</h3><div className="premium-price"><span>£</span>{plan.price}</div><p>{plan.description}</p><ul>{plan.features.map((feature) => <li key={feature}><Check size={16} aria-hidden="true" />{feature}</li>)}</ul><button className="button premium-plan-button" onClick={() => { saveCart(addCartItem(loadCart(), { id: plan.key, name: plan.name, price: Number(plan.price), href: plan.href })); go("/checkout"); }}>{plan.cta}<ArrowUpRight size={16} /></button></Reveal>)}</div></div></section>
