...
function ContactPage() { const [status, setStatus] = useState(""); const [sending, setSending] = useState(false); const submit = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = event.currentTarget; const formData = new FormData(form); setSending(true); setStatus(""); try { const response = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(formData.entries())) }); const result = await response.json(); if (!response.ok) throw new Error(result.message || "Unable to send message."); setStatus(result.message); form.reset(); } catch (error) { setStatus(error instanceof Error ? error.message : "We could not send your message right now. Please email support directly."); } finally { setSending(false); } }; return <main className="detail-page"><div className="container narrow"><span className="eyebrow">INSP AUTO / SUPPORT DESK</span><h1>Keep the next step <em>clear.</em></h1><p className="lead">Email support@inspauto.com with your question, VIN or registration, and order number if relevant.</p><form className="contact-form" onSubmit={submit}><label>Name<input required name="name" /></label><label>Email<input required type="email" name="email" /></label><label>VIN / Registration<input name="vehicle" /></label><label>Order Number<input name="order" /></label><label>Subject<input required name="subject" /></label><label>Message<textarea required name="message" rows={6} /></label><button className="button" type="submit" disabled={sending}>{sending ? "Sending…" : "Send Message ↗"}</button>{status && <p className="contact-status" role="status">{status}</p>}</form></div></main>; }

function SimplePage({ path }: { path: string }) { if (path === "/about") return <main className="detail-page"><div className="container narrow"><span className="eyebrow">INSP AUTO / ABOUT</span><h1>Read the evidence before you <em>commit.</em></h1><p className="lead">We help UK vehicle buyers look beyond the sales pitch.</p><p>INSP AUTO presents vehicle history information in a format that is easier to understand before you buy. Our service is built around clear reporting, careful language, and practical customer support—not inflated promises.</p><div className="detail-grid"><div><strong>01</strong><span>Buyer-first reporting</span></div><div><strong>02</strong><span>Plain-language evidence</span></div><div><strong>03</strong><span>Support when you need it</span></div></div><button className="button" onClick={() => navigate("/services")}>Explore services <span>↗</span></button></div></main>; if (path === "/contact") return <ContactPage />; return <NotFound />; }
